...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function calculatePackaging() {
    ...
    const l = parseFloat(calcLength.value) || 30;
    const w = parseFloat(calcWidth.value) || 20;
    const h = parseFloat(calcHeight.value) || 15;
    const ply = calcPly ? calcPly.value : '5ply';

    // Volumetric weight in kg (L x W x H in cm / 5000)
    ...
    if (ply === '3ply') {
      flute = (l + w < 40) ? 'E-Flute (Micro)' : 'B-Flute (Durable)';
  ...
  const productCards = document.querySelectorAll('.product-filter-card');
